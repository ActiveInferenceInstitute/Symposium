...
def read_markdown_files(folder_path: Path, pattern: str) -> Tuple[List[str], List[str], List[str]]:
    """Read all Markdown files matching the pattern in the given folder and its subfolders."""
    documents = []
    filenames = []
    labels = []
    ...
    try:
        ...
        for file_path in matching_files:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    # Extract meaningful label based on file type
                    if 'fieldshift' in str(file_path):
                        # For fieldshift files, use participant name and domain
                        label = f"{file_path.parent.parent.name}_{file_path.stem.split('_')[-2]}"
                    elif 'catechisms' in str(file_path):
                        # For catechism files, use participant name and domain
                        label = f"{file_path.parent.parent.name}_{file_path.stem.split('_')[-2]}"
                    elif 'collaborative_proposals' in str(file_path):
                        # For collaborative proposals, use both participant names
                        parts = file_path.stem.split('_x_')
                        label = f"{parts[0]}_{parts[1]}"
                    else:
                        # For research profiles, use participant name
                        label = file_path.parent.name
                    
                    labels.append(label)
                    logger.debug(f"Read file: {file_path} with label: {label}")
        ...
        logger.error(f"Error accessing path {folder_path}: {str(e)}")
    
    if not documents:
        logger.warning(f"No documents found in {folder_path} matching pattern {pattern}")
...
def perform_tfidf_and_dim_reduction(documents: List[str], n_components: int = None) -> Tuple:
    """Perform TF-IDF vectorization and dimensionality reduction with adaptive parameters"""
    try:
        vectorizer = TfidfVectorizer(max_features=5000, stop_words='english')
        tfidf_matrix = vectorizer.fit_transform(documents)
        
        # Adaptively set n_components based on data size
        n_samples = len(documents)
        n_features = tfidf_matrix.shape[1]
        max_components = min(n_samples, n_features) - 1  # Leave room for numerical stability
        
        if n_components is None or n_components > max_components:
            n_components = max_components
            logger.info(f"Adjusted n_components to {n_components} based on data size")
        
        # Perform dimensionality reduction
        pca = PCA(n_components=n_components, random_state=42)
        lsa = TruncatedSVD(n_components=n_components, random_state=42)
        
        # For t-SNE, adjust perplexity based on sample size
        # Perplexity should be smaller than the number of samples
        perplexity = min(30, n_samples - 1)  # Default is 30, but ensure it's valid
        ...
        try:
            ...
            if n_samples > 3:  # t-SNE needs at least 4 samples
                tsne_result = tsne.fit_transform(tfidf_matrix.toarray())
            else:
                logger.warning("Not enough samples for t-SNE visualization")
                tsne_result = None
            
            logger.info(f"Successfully performed dimensionality reduction: PCA shape={pca_result.shape}")

    ...
    try:
        ...
        for participant_dir in base_dir.iterdir():
            if participant_dir.is_dir():
                # Updated profile path to match your file naming convention
                profile_path = participant_dir / f"{participant_dir.name}_research_profile.md"
                ...
                if profile_path.exists():
                    ...
                    logger.warning(f"Profile not found for {participant_dir.name}: {profile_path}")

        if participant_data:
            logger.info(f"📚 Loaded data for {len(participant_data)} participants")
        else:
            logger.error("No participant data found. Checked in: {base_dir}")
...
def generate_project_prompt(participant_name, participant_info):
    ...
    catechism = read_description('Catechism/EUGrants.md')
    participant = participant_info['profile']
    return f"""You are top researcher in soft matter research field with the expertise in writing successful projects with the focus on innovation and creativity. The projects have to address important challenges and be breakthrough in nature. However they should be very detailed and technically sound and with high feasibility. Use the technique of Research Domain Field Shift. Concepts from one domain apply to another to create a shifted domain with new original solution to a problem. For the {participant} you need to create a new and highly original project in the field "{field}" finding solutions to future challenges in the field of intelligent soft matter. Select most suitable 5 collaborators for the project from the list "{collaborators}". Do not reply immediately. Analyse and choose the most reasonable project and setup.
